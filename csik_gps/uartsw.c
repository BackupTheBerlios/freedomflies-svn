...
void uartswRxBitService(void)
{
	...
	else
	{
		...
		if(UartswRxBitNum >= 8)
		{
			...
			// if \r then add to our 'sentence' buffer
			if(UartswRxData == ',')
			{
				if(uartswRxBuffer.datalength<8)
				{
					bufferAddToEnd(&uartswRxBuffer, UartswRxData);
					bufferAddToEnd(&uartswRxBuffer, 'q');
					bufferAddToEnd(&uartswRxBuffer, ' ');
				}
				else
				{
					bufferAddToEnd(&uartswRxBuffer, UartswRxData);
					bufferAddToEnd(&uartswRxBuffer, 'w');
					bufferAddToEnd(&uartswRxBuffer, ' ');
				}
			}
			else if(UartswRxData == '\r')
			{
				...
				{
					sentence[c] = bufferGetFromFront(&uartswRxBuffer);
				}
				sentence[c] = '\0';
				...
				if(UartswRxData != '\n') bufferAddToEnd(&uartswRxBuffer, UartswRxData); // save data in receive buffer
			// disable OC1B interrupt
			cbi(TIMSK, OCIE1B);
